...
namespace Inventiv.Todo.Module.TaskManagement
{
    ...
    public class Tasks : Query<Task>, ITasksService
    {
        ...
        internal List<Task> ByColumn(Column column) => By(t => t.Column == column);

        internal List<Task> ByUser(User user) => By(t => t.User == user);

        internal List<Task> TakeBy(int maxRowCount, DateTime maxDueDate, bool reminded) => By(t => t.DueDate < maxDueDate && t.Reminded == reminded,
                take: maxRowCount
            );
