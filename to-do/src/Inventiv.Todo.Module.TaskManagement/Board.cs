...
namespace Inventiv.Todo.Module.TaskManagement
{
	public class Board : IBoardService, IBoardInfo, IBoardDetail
	{
		...
		public virtual Column AddColumn(string name)
		{
			return context.New<Column>().With(this, name);
		}

		public virtual List<Column> GetColumns()
		{
			return context.Query<Columns>().ByBoard(this);
		}

		public virtual void AddUser(User user)
		{
			context.New<UserBoard>().With(this, user);
		}
		...
		{
			context.Query<UserBoards>().SingleBy(this, user)?.Delete();
		}

		protected internal virtual bool HasUser(User user)
		{
			return context.Query<UserBoards>().CountBy(this, user) > 0;
		}
	...
	public class Boards : Query<Board>, IBoardsService
	{
		...
		internal List<Board> ByName(string name)
		{
			return By(b => true, //default where clause always returns true so that when name is not given it will list all the boards
