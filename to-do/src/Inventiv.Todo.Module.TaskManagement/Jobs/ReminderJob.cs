...
namespace Inventiv.Todo.Module.TaskManagement.Jobs
{
    public class ReminderJob : IScheduledJob // Job classes should implement this interface to be registered as sheduled jobs
    {
        ...
        public void Execute()
        {
            ...
            foreach (var task in context.Query<Tasks>().TakeBy(BatchCount, context.System.Today.AddDays(1).ToDateTime(), false))
            {
                task.RemindUser();
            }
        }
