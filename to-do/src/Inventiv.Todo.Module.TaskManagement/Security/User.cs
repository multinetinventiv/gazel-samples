...
namespace Inventiv.Todo.Module.TaskManagement.Security
{
	public class User : IUserService, IUserInfo, IAccount //User implements IAccount interface so that it can be used by Gazel for authentication and authorization
	{
		...
		protected internal virtual User With(string name, Email email, Password password)
		{
			...
			if (email.IsDefault()) { throw new TaskManagementException.RequiredParameterIsMissing(nameof(email)); }
			if (password.IsDefault()) { throw new TaskManagementException.RequiredParameterIsMissing(nameof(password)); }
		...
		protected internal virtual UserSession CreateNewSession()
		{
			return context.New<UserSession>().With(this);
		}

		public virtual List<Task> GetTasks()
		{
			return context.Query<Tasks>().ByUser(this);
		}
	...
	public class Users : Query<User>, IUsersService
	{
		...
		internal User SingleBy(Email email, Password password)
		{
			return SingleBy(u => u.Email == email && u.Password == password);
		}

		internal List<User> ByName(string name)
		{
			return By(b => true, //default where clause always returns true so that when name is not given it will list all the users
